package com.tk.edu.lib_video.video.manager;
...
public class CustomManager extends GSYVideoBaseManager {
    ...
    private static final Map<String, CustomManager> sMap = new HashMap<>();
    ...
    public static void mute() {
        if (sMap.size() > 0) {
            for (Map.Entry<String, CustomManager> header : sMap.entrySet()) {
                if (header.getValue().needMute) {
                    continue;
                }
                header.getValue().setNeedMute(true);
            }
    ...
    public static void onResumeAll(boolean seek) {
        if (sMap.size() > 0) {
            ...
            Log.d(TAG, "onResumeAll() called with: seek = [" + seek + "]");
        }
    }
    ...
    public static void clearAllVideo() {
        if (sMap.size() > 0) {
            ...
            Log.d(TAG, "clearAllVideo() called");
        }
        sMap.clear();
    }
